            ...
            <div class="row">
                <div class="col-12">
                    <div class="mainTitle lightTitle">
                        ...
                        <h1 class="sectionSubTitle"><?=$current_cat->name?>
                            <span class="shadowText"><?=$current_cat->name?></span>
                        </h1>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-12 col-xl-10 offset-xl-1">
                        <div class="blogNavWrap dragscroll">
                            <ul>
                                <li><a href="/blog" class="">Все<span></span></a></li>
                                <?php foreach($cats as $cat): if($cat->term_id == 1) continue;?>
                                    <li><a href="<?=(!kubster_is_active_cat($cat))?get_term_link($cat):'#'?>" class="<?=(kubster_is_active_cat($cat))?'active':''?>"><?=$cat->name?><span></span></a></li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
                    </div>
        ...
        <?php if(!empty($secondBlogPart)): ?>
            <div class="blogBottom">
                <div class="container">
                    ...
                    <div class="row">
                        <div class="col-12 col-xl-10 offset-xl-1">
                            <div class="row load_more_wrapper">
...
<script>
    ...
    $('.load_more_btn').click(function(e){
        e.preventDefault();
        $(this).text('<?php string_translate('Loading..', 'Загрузка...')?>');
        $.get('/wp-admin/admin-ajax.php?action=klm&category='+category+'&page='+page, {}, function (data) {
            ...
            $('.load_more_btn').text('<?php string_translate('Load more', 'Загрузить еще');?>');
        });
        return false;
    });
</script>
